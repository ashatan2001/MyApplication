...
class AuthInterceptor
    ...
    constructor(
        ...
        private val authApiProvider: Provider<AuthApi>, // 2. Используем Provider вместо прямого внедрения
    ) : Interceptor {
        ...
        override fun intercept(chain: Interceptor.Chain): Response {
            ...
            return runBlocking(Dispatchers.IO) {
                mutex.withLock {
                    try {
                        // 3. Получаем экземпляр AuthApi только здесь
        ...
        private suspend fun handleAuthFailure() {
            cookieJar.clear()
            localDataSource.clearSession() // Очищаем DataStore -> UI автоматически узнает о выходе
        }
